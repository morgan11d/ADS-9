...
int main() {
    ...
    for (int n = 2; n <= 9; ++n) {
        ...
        guard += all.size();
        double allMs = std::chrono::duration<double, std::milli>(finishAll - startAll).count();
        ...
        double perm1Ms = std::chrono::duration<double, std::milli>(finishPerm1 - startPerm1).count() / numbers.size();
        ...
        double perm2Ms = std::chrono::duration<double, std::milli>(finishPerm2 - startPerm2).count() / (numbers.size() * repeats);

        rows.push_back({n, allMs, perm1Ms, perm2Ms});
        std::cout << n << " " << allMs << " " << perm1Ms << " " << perm2Ms << std::endl;
    }

    std::ofstream csv("result/data.csv");
    csv << "n,getAllPerms_ms,getPerm1_ms,getPerm2_ms\n";
    for (const Row& row : rows) {
        csv << row.n << "," << row.allMs << "," << row.perm1Ms << "," << row.perm2Ms << "\n";
    }
